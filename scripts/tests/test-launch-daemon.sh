...
launcher_script="$(python3 - "$qml_file" <<'PY'
import re, sys, pathlib

qml_path = pathlib.Path(sys.argv[1])
content = qml_path.read_text(encoding="utf-8")

env_match = re.search(r'readonly property string _envRestoreScript:\s*`([^`]+)`', content)
if not env_match:
    sys.exit(f"Error: _envRestoreScript not found in {qml_path}")
env_script = env_match.group(1)

daemon_match = re.search(r'function launchDaemon\(opts:\s*var\): void\s*\{.*?const script = root\._envRestoreScript \+\s*`([^`]+)`', content, re.DOTALL)
if not daemon_match:
    sys.exit(f"Error: launchDaemon script not found in {qml_path}")
daemon_script = daemon_match.group(1)

full_script = env_script + "\n" + daemon_script
# Unescape JS template literal escapes (\${...} -> ${...}, \` -> `)
unescaped_script = re.sub(r'\\(\$|\`)', r'\1', full_script)
print(unescaped_script)
PY
)"
...
invoke_launch_daemon() {
    ...
    bash -lc "$launcher_script" "inir-daemon" "$custom_systemd_run" "$desc" "$unit" "$restart" "$program" "${extra_args[@]}"
}
...
set +e
output="$(invoke_launch_daemon "test-dummy" "sleep" "" "Test Dummy Daemon" "/usr/bin/systemd-run" "60" 2>&1)"
ec=$?
set -e
...
# Scenario 2: ALREADY_RUNNING (Idempotent launch does not spawn duplicate)
log_step "Scenario 2: ALREADY_RUNNING"
...
if [[ $ec -ne 0 ]]; then
    log_fail "Scenario 2: Expected exit code 0 for already-running daemon, got $ec. Output: $output"
fi

pid_after="$(systemctl --user show -p MainPID --value inir-test-dummy.service)"
if [[ "$pid_before" != "$pid_after" ]]; then
    log_fail "Scenario 2: MainPID changed unexpectedly from $pid_before to $pid_after"
fi

# Ensure no second sleep process was spawned in shell cgroup
matching_pids=($(pgrep -f "sleep 60" || true))
found_main_pid=false
...
if [[ "$found_main_pid" != true ]]; then
    log_fail "Scenario 2: dummy_pid $dummy_pid not found in running processes"
fi

log_pass "Scenario 2 (ALREADY_RUNNING): Idempotency verified, PID preserved ($dummy_pid)"
...
set +e
output="$(invoke_launch_daemon "test-rel" "sleep" "" "Test Rel Daemon" "/usr/bin/systemd-run" "60" 2>&1)"
ec=$?
set -e
...
set +e
err_out="$(invoke_launch_daemon "test-notfound" "nonexistent_bin_xyz_999" "" "Test Not Found" "/usr/bin/systemd-run" 2>&1)"
ec=$?
set -e
...
set +e
err_out="$(invoke_launch_daemon "test-fail" "sleep" "" "Test Fail Daemon" "$mock_systemd_run" "59" 2>&1)"
ec=$?
set -e
...
set +e
output="$(invoke_launch_daemon "test-crash" "false" "on-failure" "Test Crash Daemon" "/usr/bin/systemd-run" 2>&1)"
ec=$?
set -e
...
set +e
output="$(PATH="$no_systemd_bin_dir" invoke_launch_daemon "test-nosystemd-success" "true" "" "Test No Systemd Success" "/nonexistent/systemd-run" 2>&1)"
ec=$?
set -e
...
set +e
output="$(PATH="$no_systemd_bin_dir" invoke_launch_daemon "test-nosystemd-fail" "sh" "" "Test No Systemd Fail" "/nonexistent/systemd-run" "-c" "exit 42" 2>&1)"
ec=$?
set -e
...
log_step "All 7 contract scenarios passed successfully"
